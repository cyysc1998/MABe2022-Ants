...
class PCLModel(BaseModel):
    ...
    def optimize_parameters_amp(self, current_iter):
        ...
        with autocast():
            ...
            logits, labels, output_proto, target_proto = self.net(self.x11, self.x12, self.x21, self.x22)
            ...
            if output_proto is not None:
                ...
                for proto_out,proto_target in zip(output_proto, target_proto):
                    loss_proto += cross_entropy(proto_out, proto_target)  
                    
                # average loss across all sets of prototypes
                loss_proto /= len(self.opt["num_cluster"]) 
                loss_dict["loss_proto"] = loss_proto
                loss += loss_proto   
