...
class S3D_(nn.Module):
    def __init__(self, num_class):
        super(S3D_, self).__init__()
...
class S3D(nn.Module):
    def __init__(self, opt):
        ...
        self.encoder = S3D_(opt["num_class"])
        weight = {k.replace('module.',''):v for k,v in torch.load(opt["pretrained_model"]).items()}
    ...
    def fit_input(self, x):
        ...
        (B, T, H, W) -> (B, C, T, H, W)
        '''
        return x.unsqueeze(1).repeat(1, 3, 1, 1, 1)
        

    def forward(self, x_list):
        x_list = [self.encoder(self.fit_input(x)) for x in x_list]
        return [self.fc(x) for x in x_list]
...
#     net = S3D_(10).cuda()
#     x = torch.rand(16, 3, 7, 224, 224).cuda()
#     macs, param = profile(net, inputs=(x,))
#     macs, param = clever_format([macs, param], "%.3f")
#     print(macs)
#     print(param)