...
def main():
    ...
    logger, tb_logger = init_loggers(
        opt, prefix="val", log_level=logging.INFO, use_tb_logger=False
    )
    ...
    for current_iter in range(
        opt["val"]["st_iter"], opt["val"]["ed_iter"] + 1, opt["val"]["val_freq"]
    ):
        ckpt_path = f"{opt['path']['models']}/net_{current_iter}.pth"
        model.load_network(model.net, ckpt_path)
        model.test(val_set, val_loader)
        model.save_result(-1, current_iter, "val")

    logger.info(f"End of validation.")
